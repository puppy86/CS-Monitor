...
{
    public class TestApi2Controller : Controller
    {
        ...
        public TestApi2Controller(IIndexService indexService, INodesService nodesService, IGraphService graphService)
        {
        ...
        public string Balance(string id)
        {
            using (var client = CreateApi())
            {
                var balance = client.BalanceGet(Base58Encoding.Decode(id));
        ...
        public TokenAmounts AccountTokens(string id, string tokens)
        {
            ...
            {
                result.Tokens.Add(new TokenAmount { Token = token, Value = "Not supported in this network" });
            }
            return result;
        }
